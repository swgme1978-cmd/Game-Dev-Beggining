...
8. **Documentation Management:** You are responsible for maintaining a `Documentation_Index.md` file in the `/docs` folder. You must update it constantly by adding a new entry and a brief, useful summary for every new document created in the `/docs` folder, and by updating summaries if any existing document is significantly amended. This ensures the index always provides an accurate, current overview of all project documentation for future sessions to reference.
